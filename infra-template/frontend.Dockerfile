...
# The backend URL is NOT baked in at build time. It is injected at runtime via
# /config.js (see frontend-entrypoint.sh), so this image is built once and
# targets any backend. The build is therefore backend-agnostic.
RUN npm run build
